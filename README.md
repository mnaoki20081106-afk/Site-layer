...
- Workers Static Assets はデフォルトで、静的ファイルに一致するリクエスト（`/admin.html` など）を Worker のコードを経由せず直接返します。そのため `wrangler.toml` に `run_worker_first` を指定していないと、`ADMIN_TOKEN` 埋め込みやOGP書き換えのロジックが一度も実行されません（このリポジトリは `run_worker_first = ["/", "/index.html", "/admin.html"]` を設定済みです）。
