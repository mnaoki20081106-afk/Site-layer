...
サイト①のHTMLをそのまま返す方式のため、`og:title` 等のOGPタグは基本的にサイト①のものがそのまま使われます（追加の取得処理は不要）。`og:image` / `twitter:image` だけは、管理画面でサイト①のURLを実際に変更したタイミングのタイムスタンプを付与（`?_v=...`）するので、LINEやX（Twitter）などが画像を古いまま(キャッシュ)表示し続けることを防げます。`og:url` はこのサイト自身のURLに書き換えます。
