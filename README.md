...
サイト②（前面）の重ね範囲がサイト①の一部を覆っていると、その範囲内のクリックはサイト②が受け取り、サイト①側のリンクなどには届きません。管理画面の「サイト②の表示位置・サイズ」を、実際に重ねたい要素だけを覆う小さな範囲に絞ってください。範囲外は常にサイト①へのクリックがそのまま届きます。

また、画面右下に常時「元のサイトを開く」というリンクを表示しており、これを押すとサイト①を新しいタブで開けます（重なりに関係なく確実にサイト①へ遷移できる導線です）。
